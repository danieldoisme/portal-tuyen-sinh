    ...
    return text.split("\n").map((line, idx) => {
      ...
      if (headingMatch) {
        ...
        return (
          ...
          >
            {level === "h2" ? (
              <h2 className="text-2xl font-bold text-red-700 mt-8 mb-2">{line}</h2>
            ) : (
              <h3 className="text-lg font-semibold text-gray-800 mt-4">{line}</h3>
            )}
          </div>
        );
      }
      ...
      return <p key={idx} className="text-gray-700 mb-1">{line}</p>;
    });
  };
