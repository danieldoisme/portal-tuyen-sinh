...
const postgraduateAdmissionsData = [
  {
    image:
...
const AnnouncementsPostgraduatePage = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 12;

  const totalPages = Math.ceil(
    postgraduateAdmissionsData.length / itemsPerPage
  );
  ...
  const selectedData = postgraduateAdmissionsData.slice(
    startIndex,
...
export default AnnouncementsPostgraduatePage;
