...
const ThongBaoChiTiet = () => {
    ...
    const fetchArticle = async () => {
      ...
      try {
        ...
        if (result.status === "success" && result.data) {
          ...
          setErrorMsg(result.message || "Không tìm thấy thông báo");
        }
      } catch (err) {
    ...
    const fetchRelated = async () => {
      try {
        const res = await fetch(
          `http://localhost:8081/api/articles/thong-bao/tuyen-sinh-dai-hoc/1`
        );
        const result = await res.json();
        if (result.status === "success" && result.data) {
  ...
  if (!article) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        <h1 className="text-2xl font-bold">{errorMsg || "Không tìm thấy thông báo"}</h1>
        <Link
            ...
            <div className="sticky top-24">
              ...
              <div className="space-y-6">
                {relatedPosts.map((post) => (
                  <Link
                    to={`/thong-bao/tuyen-sinh-dai-hoc/${post.id}`}
                    key={post.id}
